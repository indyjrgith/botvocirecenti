-- Modulo:VociRecenti
-- Versione 8.48 - Dati condivisi tra istanze nella stessa pagina:
--                loadAllData usa mw.loadData() per Modulo:VociRecenti/DatiN.
--                Scribunto valuta ogni pagina Dati una sola volta per pagina
--                e la espone come tabella immutabile: piu' #invoke con filtri
--                diversi non ricaricano ognuno la cache completa. I test di
--                lista vuota non usano #, non affidabile sui proxy loadData.
-- Versione 8.47 - Ottimizzazione memoria per le chiamate senza filtri:
--                se Order=Data, Timestamp=off/date e First non e' specificato,
--                vengono caricate automaticamente solo Num voci. E' equivalente
--                al risultato completo perche' la cache e' scritta dal bot in
--                ordine decrescente di data; evita di materializzare Dati2...
--                DatiN per le normali liste delle ultime voci. Nei casi con
--                filtri, ordinamenti alternativi o calcoli di intervallo date
--                il caricamento resta completo, cosi' il risultato non cambia.
...
local function matchesNoCat(categories, noCat)
    ...
    if mw.text.trim(noCat) == '*' then
        return not categories or categories[1] == nil
    end
    -- Pre-lowercase categorie una volta sola
    local catsLow = {}
    for i, cat in ipairs(categories) do
...
local function matchesNoTemplate(templates, noTmpl)
    ...
    if mw.text.trim(noTmpl) == '*' then
        return not templates or templates[1] == nil
    end
    -- splitParens: rispetta i separatori dentro not(...), es. not(X;Y) o not(X,Y)
...
-- Dati caricati con mw.loadData: una tabella grande viene valutata una sola volta
-- per pagina, anche con piu' #invoke. Il risultato e' immutabile.
    ...
    while i <= 100 and not done do
        ...
        if dataPage and dataPage.exists then
            local success, data = pcall(function()
                return mw.loadData(pageName)
            end)
            if success then
                if type(data) == 'table' then
                    local vociList = data.d or data.voci
                    if vociList and type(vociList) == 'table' and vociList[1] ~= nil then
...
function p.main(frame)
    ...
    -- Invarianti della chiamata: non ricalcolarle per ogni voce della cache.
    local hasPositiveFilters = (andCat ~= '' or orCat ~= '' or hAndCat ~= '' or hOrCat ~= ''
        or titleSearch ~= '' or searchText ~= '' or regexPattern ~= ''
        or andTemplate ~= '' or orTemplate ~= '' or portali ~= '' or orPortali ~= '')
    local hasNegativeFilters = (noCat ~= '' or hNoCat ~= '' or noTemplate ~= '')
    local hasAnyDataFilter = hasPositiveFilters or hasNegativeFilters or dataFine ~= ''
        or exclIfNotCat ~= ''
    ...
    -- Caso comune: lista delle ultime Num voci senza filtri. La cache viene
    -- scritta in ordine decrescente, quindi i dati oltre Num non modificano
    -- l'output. Timestamp=days/on/cachedays resta completo per non alterare
    -- il calcolo dell'intervallo temporale.
    if not firstParam and not hasAnyDataFilter and order == 'data'
            and (timestampParam == 'off' or timestampParam == 'date') then
        firstParam = num
    end

    local data = loadAllData(firstParam)
    local vociSlice = data.voci

    if not data or not vociSlice or #vociSlice == 0 then
    ...
    if displayNum == 0 then
        ...
        if timestampParam ~= 'off' and data.ultimo_aggiornamento then
            msg = msg .. '\n' .. buildTimestampLine(timestampParam, data.ultimo_aggiornamento, data.num_files, nil, vociSlice)
        end
        return msg
    end
    ...
    if scrollHeight then
        ...
        if timestampParam ~= 'off' and data.ultimo_aggiornamento then
            result = result .. '\n' .. buildTimestampLine(timestampParam,
                     data.ultimo_aggiornamento, data.num_files, filtered, vociSlice)
        end
    else
        result = p.formatOutput(filtered, disp, data.ultimo_aggiornamento, data.num_files, timestampParam, vociSlice, displayNum)
    end
    return result
end
...
function p.debugVoce(frame)
    ...
    table.insert(output, "'''Numero categorie:''' " .. listLength(voceTest[3]) .. "\n\n")
    ...
    table.insert(output, "\n'''Numero template:''' " .. listLength(templates) .. "\n")
    table.insert(output, "\n'''Template:'''\n")
    ...
    table.insert(output, "* Versione modulo: 8.44\n")
    for i, tmpl in ipairs(templates) do
        local t1 = tmpl[1]
        local t2 = tmpl[2]
    ...
    if type(voceTest[4]) == "table" then
        table.insert(output, "* voceTest[4] length: " .. listLength(voceTest[4]) .. "\n")
    end
    ...
    if noTmplTest ~= "" then
        ...
        table.insert(output, "* listLength(templates)=" .. listLength(templates) .. "\n")
        local result = (trimmed == "*") and (listLength(templates) == 0) or true
        table.insert(output, "* matchesNoTemplate result: " .. tostring(trimmed == "*" and listLength(templates) == 0) .. "\n")
    end
