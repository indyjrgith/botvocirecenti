-- Modulo:VociRecenti
-- Versione 8.12 - Supporto ";" come separatore OR nei parametri Title, Text,
...
-- Versione 8.11 - Aggiunto parametro |DispScroll= per lista in div scrollabile
-- Versione 8.10 - (interna)
...
-- Verifica se una stringa (haystack) contiene almeno uno dei termini
-- separati da ";" (OR). Usata internamente dalle funzioni di matching.
...
local function matchesNoCat(categories, noCat)
    ...
    -- Es: "medici;biologi,storici" -> esclude se (medici OR biologi) E storici
...
local function matchesTitle(title, titleSearch)
    ...
    -- Supporta | o ; per OR: "Oasi;parco;riserva" oppure "Oasi{{!}}parco{{!}}riserva"
    if mw.ustring.find(titleSearch, '|', 1, true) or mw.ustring.find(titleSearch, ';', 1, true) then
        -- Normalizza: sostituisce | con ; per usare split su ; (| è metacarattere Lua)
...
local function matchesText(content, text)
    ...
    -- Supporta | o ; per OR: "gotico;romanico;barocco" oppure "gotico{{!}}romanico{{!}}barocco"
...
local function matchesOrTemplate(templates, orTmpl)
    ...
    -- Virgola = OR tra gruppi; punto e virgola = OR dentro ogni gruppo (entrambi OR)
