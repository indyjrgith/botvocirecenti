...
local function matchesRegex(content, pattern)
    ...
    -- Converti sia contenuto che pattern in lowercase per case-insensitive
    local contentLower = mw.ustring.lower(content)
    local patternLower = mw.ustring.lower(pattern)
...
local function loadAllData()
    ...
    while i <= 100 do  -- Max 100 file (sicurezza)
        ...
        if dataPage and dataPage.exists then
            ...
            -- Verifica che il file contenga effettivamente dati (non sia svuotato)
            if success and data and data.voci and #data.voci > 0 then
...
function p.main(frame)
    ...
    if not data.voci or #data.voci == 0 then
        if data.num_files == 0 then
            -- Nessun file cache trovato
            return '<div class="noprint" style="padding:1em; border:2px solid #fc3; background:#ffc;">\'\'\'⚠️ Cache non inizializzata\'\'\'<br/>Esegui il bot per creare la cache iniziale.</div>'
        else
            -- File trovati ma vuoti
            return '<div class="noprint" style="padding:1em; border:2px solid #fc3; background:#ffc;">\'\'\'⚠️ Cache vuota\'\'\'<br/>Il bot sta rigenerando la cache. Riprova tra qualche minuto.</div>'
        end
    end
    ...
    for _, voce in ipairs(data.voci) do
        ...
        if useAndLogic then
            ...
            if not hasFilters then
                ...
                if regexPattern ~= '' and matchesRegex(voce.contenuto, regexPattern) then
                    matches = true
                end
            end
