...
launch_bot() {
    ...
    if command -v tmux &>/dev/null; then
        ...
        if tmux has-session -t botvoci 2>/dev/null; then
            ...
            tmux new-window -t botvoci -n "bot-$(date +%H%M)" "$BOT_CMD; echo ''; echo '[Bot terminato - premi un tasto]'; read"
            tmux select-window -t botvoci
        else
            # Nuova sessione tmux
            tmux new-session -d -s botvoci -n "bot" "$BOT_CMD; echo ''; echo '[Bot terminato - premi un tasto]'; read"
        fi
...
install_crontab() {
    ...
    echo "PYTHONPATH=/data/data/com.termux/files/usr/lib/python3.13/site-packages HOME=/data/data/com.termux/files/home ${cron_expr} ${BOT_CMD} >> ${BOT_DIR}/bot.log 2>&1 ${CRON_TAG}" >> "$tmpfile"
    crontab "$tmpfile"
    rm -f "$tmpfile"
