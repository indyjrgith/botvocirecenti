...
local function splitParens(str, sep)
    ...
    while i <= len do
        ...
        elseif c == sep and depth == 0 then
            local token = mw.text.trim(table.concat(current))
            if token ~= '' then
...
function p.main(frame)
    ...
    for _, voce in ipairs(data.voci) do
        ...
        if useAndLogic then
            ...
            if hAndCat ~= '' and not matchesAndCat(voce[7] or {}, hAndCat) then matches = false end
            if hOrCat ~= '' and not matchesOrCat(voce[7] or {}, hOrCat) then matches = false end
...
function p.formatOutput(voci, disp, ultimoAggiornamento, numFiles, timestampParam, cacheVoci)
    ...
    elseif disp == 'o' then
        for i, voce in ipairs(voci) do
            local dateStr = formatTimestampDisplay(voce)
