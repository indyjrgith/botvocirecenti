-- Modulo:VociRecenti
-- Versione 8.4 - Nuovo formato cache compatto (array posizionale), AndTemplate/OrTemplate/Portali, TextRegExp su stringa concatenata
...
local function matchesRegex(content, pattern, caseSensitive)
    ...
    -- Se NON case-sensitive, converti il contenuto in lowercase
...
function p.main(frame)
    ...
    local noCat = args.NoCat or ''
    local titleSearch = args.Title or ''
    local searchText = args.Text or ''
    local regexPattern = args.TextRegExp or ''
    local andTemplate = args.AndTemplate or ''
    local orTemplate = args.OrTemplate or ''
    local portali = args.Portali or ''
    ...
    for _, voce in ipairs(data.voci) do
        ...
        if useAndLogic then
            ...
            local hasFilters = (andCat ~= '' or orCat ~= '' or titleSearch ~= '' or searchText ~= '' or regexPattern ~= '' or andTemplate ~= '' or orTemplate ~= '' or portali ~= '')
