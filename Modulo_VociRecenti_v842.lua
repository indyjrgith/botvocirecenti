-- Modulo:VociRecenti
-- Versione 8.42 - Ottimizzazioni performance (nessuna modifica alla logica):
--                1) splitParens riscritta: sostituita l'iterazione carattere per carattere
--                   (str:sub(i,i) in loop) con string.find sui caratteri significativi,
--                   riducendo drasticamente il numero di iterazioni su stringhe con pochi token.
--                2) Loop principale (modalità And=on): aggiunto early exit con if/then annidati
--                   per cortocircuitare i check non appena la voce è già esclusa, evitando
--                   chiamate inutili alle funzioni di matching successive.
--                3) Troncamento lista filtered: eliminata la tabella ausiliaria 'truncated'
--                   e la ricopiatura degli elementi; il limite viene ora passato direttamente
--                   a formatOutput che itera fino a math.min(#voci, num).
...
local function matchesAndCat(categories, andCat)
    ...
    -- not(X) in un gruppo: il gruppo è soddisfatto se NESSUNA categoria matcha X
    -- (opera sull'insieme: non basta che "una categoria non matchi X")
    -- splitParens: rispetta i separatori dentro not(...), es. not(X;Y) o not(X,Y)
    local groups = splitParens(andCat, ',')
    for _, group in ipairs(groups) do
        local found = false
        -- Il gruppo contiene termini separati da ";": OR tra di essi
        ...
        for _, term in ipairs(terms) do
            ...
            if term ~= '' then
                ...
                if isNot then
                    -- not(X;Y) o not(X,Y): soddisfatto se NESSUNA categoria matcha
                    -- almeno uno dei sotto-termini (OR tra sotto-termini da negare)
                    local subTerms = expandNotInner(inner)
                    local anyMatch = false
                    for _, subTerm in ipairs(subTerms) do
                        for _, cat in ipairs(categories) do
                            if contains(cat, subTerm) then anyMatch = true; break end
                        end
                        if anyMatch then break end
                    end
                    if not anyMatch then found = true; break end
                else
                    -- normale: soddisfatto se almeno una categoria matcha
                    for _, cat in ipairs(categories) do
                        if contains(cat, term) then found = true; break end
                    end
...
local function matchesOrCat(categories, orCat)
    ...
    -- not(X): questo termine OR è soddisfatto se NESSUNA categoria matcha X
    -- splitParens: rispetta i separatori dentro not(...), es. not(X;Y) o not(X,Y)
    ...
    for _, group in ipairs(groups) do
        ...
        for _, term in ipairs(terms) do
            ...
            if term ~= '' then
                local isNot, inner = parseNot(term)
                if isNot then
                    -- not(X;Y) o not(X,Y): match se NESSUN sotto-termine matcha
                    local subTerms = expandNotInner(inner)
                    local anyMatch = false
                    for _, subTerm in ipairs(subTerms) do
                        for _, cat in ipairs(categories) do
                    ...
                    for _, cat in ipairs(categories) do
                        if contains(cat, term) then return true end
                    end
                end
            end
        end
...
local function matchesNoCat(categories, noCat)
    ...
    -- not(X) in NoCat: inverte il senso -> richiede positivamente X
    --   (la voce viene esclusa se NON ha X, cioè passa solo se ha X)
    -- splitParens: rispetta i separatori dentro not(...), es. not(X;Y) o not(X,Y)
    local groups = splitParens(noCat, ',')
    for _, group in ipairs(groups) do
        local terms = splitParens(group, ';')
        if #terms == 0 then terms = {group} end
        -- Per il gruppo con not(): il gruppo è soddisfatto (non esclude) se
        -- la voce HA almeno uno dei termini not(). Per i termini normali il gruppo
        -- esclude se la voce li ha.
        -- Logica: scansiona tutti gli spec del gruppo (OR tra di essi):
        --   - spec normale: se una cat matcha -> return false (esclude)
        --   - spec not(X): se NESSUNA cat matcha X -> return false (esclude)
        for _, term in ipairs(terms) do
            term = mw.text.trim(term)
            if term ~= '' then
                local isNot, inner = parseNot(term)
                if isNot then
                    -- not(X;Y): richiede positivamente almeno uno dei sotto-termini
                    -- (la voce viene esclusa se NON ha nessuno dei sotto-termini)
                    local subTerms = expandNotInner(inner)
                    local hasAny = false
                    for _, subTerm in ipairs(subTerms) do
                        for _, cat in ipairs(categories) do
                            if contains(cat, subTerm) then hasAny = true; break end
                        end
                        if hasAny then break end
                    end
                    if not hasAny then return false end
                else
                    -- normale: se la voce ha questo termine, escludila
                    for _, cat in ipairs(categories) do
                        if contains(cat, term) then return false end
                    end
                end
            end
        end
...
local function matchesExclIfNotCat(categories, param)
    ...
    for _, rule in ipairs(rules) do
        ...
        if rule ~= '' then
            local gt = mw.ustring.find(rule, '>', 1, true)
            if gt then
                local triggerStr   = mw.text.trim(mw.ustring.sub(rule, 1, gt - 1))
                local exceptionStr = mw.text.trim(mw.ustring.sub(rule, gt + 1))
                -- Verifica se la voce ha il trigger X
                local hasTrigger = false
                for _, cat in ipairs(categories) do
                    if contains(cat, triggerStr) then hasTrigger = true; break end
                end
                if hasTrigger then
                    -- Verifica se ha anche l'eccezione Y
                    local hasException = false
                    for _, cat in ipairs(categories) do
                        if contains(cat, exceptionStr) then hasException = true; break end
                    end
                    -- Ha X ma non Y: escludi
                    if not hasException then return false end
                end
            end
            -- Se manca il separatore > la regola è malformata: ignorata (non esclude)
        end
    end
    return true
end
...
local function matchesSingleTemplate(templates, tmplName, requiredParams)
    ...
    tmplName = mw.ustring.lower(tmplName)
    for _, tmpl in ipairs(templates) do
        local nome = mw.ustring.lower(tmpl[1] or '')
        local nameMatch
        if wholeWordTmpl then
            nameMatch = containsWholeWord(nome, tmplName)
        else
            nameMatch = mw.ustring.find(nome, tmplName, 1, true) ~= nil
            ...
            if not nameMatch then
                local tmplLow = mw.ustring.lower(tmplName)
                local nomeLow = mw.ustring.lower(nome)
                if (tmplLow == 'portale' and nomeLow == 'portali') or
                   (tmplLow == 'portali' and nomeLow == 'portale') then
                    nameMatch = true
                end
            end
        end
...
local function isPortaleTemplate(nome)
    nome = mw.ustring.lower(mw.text.trim(nome))
...
function p.main(frame)
    ...
    for _, voce in ipairs(data.voci) do
        ...
        if useAndLogic then
            ...
            if matches and regexPattern ~= '' and not matchesRegex(buildSearchable(voce), regexPattern, useCaseSensitive) then
                matches = false
            end
            ...
            if hNoCat ~= '' and not matchesNoCat(voce[7] or {}, hNoCat) then matches = false end
            if noTemplate ~= '' and not matchesNoTemplate(voce[4] or {}, noTemplate) then matches = false end
            if exclIfNotCat ~= '' and not matchesExclIfNotCat(voce[3] or {}, exclIfNotCat) then matches = false end
        end
    ...
    if scrollHeight then
        ...
        result = p.formatOutput(filtered, disp, data.ultimo_aggiornamento, data.num_files, timestampParam, data.voci, displayNum)
    end
    return result
end
...
function p.formatOutput(voci, disp, ultimoAggiornamento, numFiles, timestampParam, cacheVoci, maxItems)
    local output = {}
...
function p.debugVoce(frame)
    ...
    table.insert(output, "* Versione modulo: 8.41\n")
    for i, tmpl in ipairs(templates) do
        local t1 = tmpl[1]
        local t2 = tmpl[2]
