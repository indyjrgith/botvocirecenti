-- Modulo:VociRecenti
-- Versione 8.11 - Aggiunto parametro |DispScroll=: stessa logica di |Disp= ma il risultato
...
function p.main(frame)
    ...
    if scrollHeight then
        -- DispScroll: genera il wikitext della sola lista (senza timestamp),
        -- lo preprocessa in HTML, lo avvolge nel div scrollabile,
        -- poi aggiunge il timestamp fuori dallo scrollbox in wikitext normale.
        local listWiki = p.formatOutput(filtered, disp, nil, nil, 'off', nil)
        -- Il newline iniziale garantisce che il primo # o * sia riconosciuto
        -- come markup di lista anche dentro un contesto <div>
        local listHtml = frame:preprocess('\n' .. listWiki)
...
-- Formattazione output HTML pura (usata da DispScroll per evitare problemi
-- Formattazione output wikitext (usata da Disp)
