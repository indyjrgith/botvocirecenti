-- Modulo:VociRecenti
-- Versione 8.50 - Rilascio esplicito dei moduli DatiN durante lo streaming:
--                dopo l'elaborazione di ogni DatiN, il modulo viene rimosso da
--                package.loaded e viene eseguito il garbage collector. Questo
--                evita che require() mantenga residenti tutte le tabelle DatiN
--                tra istanze successive della stessa pagina.
...
--                Usa require() con rilascio esplicito di package.loaded: i grandi
--                DatiN non vengono mantenuti residenti tra le istanze quando
--                vengono percorsi in streaming.
...
-- In Lua require() conserva normalmente il valore restituito in package.loaded;
-- per lo streaming questo annulla il vantaggio della lettura progressiva, perché
-- i DatiN già attraversati rimarrebbero residenti fino alla fine della pagina.
...
local function loadFilteredData(matchVoce, maxMatches)
    ...
    while i <= 100 and not done do
        ...
        if success and type(data) == 'table' then
            local vociList = data.d or data.voci
            if vociList and type(vociList) == 'table' and #vociList > 0 then
                if i == 1 then ultimoAggiornamento = data.u or data.ultimo_aggiornamento end
                filesLoaded = filesLoaded + 1
                for _, voce in ipairs(vociList) do
                    if voce.titolo ~= nil then
            ...
            -- Rilascio fondamentale per evitare l'accumulo di DatiN tra istanze.
            -- Le sole voci che hanno fatto match restano vive perché referenziate
            -- da 'filtered'; le altre possono essere raccolte dal GC.
            data = nil
            releaseDataModule(pageName)
    ...
    return { voci = filtered, ultimo_aggiornamento = ultimoAggiornamento,
        num_files = filesLoaded, errors = errors }
end
...
function p.main(frame)
    ...
    local useStreaming = not firstParam and order == 'data' and timestampParam == 'off'
