-- Modulo:VociRecenti
-- Versione 8.53 - Fix "-- giorni nella cache" con Timestamp=cachedays quando
--                num e' cosi' alto da far leggere l'intera cache in streaming
--                (es. num=50000 per mostrare tutta la cache): il calcolo dei
--                giorni faceva comunque un secondo require() mirato
--                sull'ultimo file (getLastCacheTimestamp), appena rilasciato
--                da package.loaded un istante prima nello stesso ciclo di
--                streaming. Con l'intera cache gia' trattenuta in memoria da
--                quel punto (nessun early-stop per maxMatches), quel secondo
--                require() puo' fallire (pcall lo intercetta silenziosamente,
--                presumibilmente per pressione su memoria/CPU della sandbox
--                Scribunto), facendo ricadere il conteggio nel fallback '--'
--                gia' previsto per l'ultimo file davvero irraggiungibile.
--                Fix: loadFilteredData cattura ora anche il timestamp
--                dell'ultima voce dell'ultimo file effettivamente processato
--                (simmetrico alla cattura gia' esistente sulla prima voce di
--                Dati1), esposto insieme a un flag cache_esaurita (true solo
--                se lo streaming ha raggiunto la fine naturale della cache
--                senza mai fermarsi per maxMatches). Quando cache_esaurita e'
--                true il valore gia' in memoria viene riusato al posto del
--                secondo require, eliminando sia il costo sia il punto di
--                fallimento; con num piccolo (caso tipico, streaming che si
--                ferma prima dell'ultimo file) il comportamento e il percorso
--                restano quelli precedenti, invariati.
-- Versione 8.52 - Nuovo parametro |DataMassima=GG/MM/AAAA: limite superiore
--                sulla data delle voci mostrate (complementare a DataFine,
--                che resta il limite inferiore nonostante il nome). Con
--                DataFine e DataMassima entrambi valorizzati si ottiene un
--                intervallo [DataFine, DataMassima] inclusivo su entrambi
--                gli estremi. Nuova funzione isBeforeDate, speculare a
--                isAfterDate (stesso parsing GG/MM/AAAA, confronto <=
--                invece di >=). Aggiunta in entrambi i rami di voceMatches
--                (And=on e default), nello stesso punto in cui gia' opera
--                DataFine.
--                Approfittando dell'intervento, corretto anche il filtro
--                DataFine esistente (rimasto incompleto quando la data di
--                primo spostamento in NS0/move_timestamp fu introdotta in
--                v8.39/v8.41 per ordinamento, calcDaysRange e visualizza-
--                zione): confrontava sempre e solo voce[2] (timestamp di
--                creazione), mai voce[6] (move_timestamp), disallineandosi
--                dalla data effettivamente mostrata per le voci spostate da
--                altro namespace. DataFine e il nuovo DataMassima usano ora
--                entrambi effectiveTimestamp(voce) (move_timestamp se
--                presente, altrimenti timestamp di creazione), coerente-
--                mente con isAfterDate/isBeforeDate difese anche contro un
--                timestamp nil (return false anziche' errore su sub()).
...
-- Speculare a isAfterDate: usata dal filtro DataMassima per escludere le
-- voci piu' recenti del limite superiore. Stesso formato data (GG/MM/AAAA),
-- stesso confronto su stringa YYYYMMDD, confronto invertito (<=).
local function isBeforeDate(timestamp, dateLimit)
    if not dateLimit or dateLimit == '' then return true end
    if not timestamp then return false end
    local day, month, year = dateLimit:match('(%d+)/(%d+)/(%d+)')
    if not day then return true end
...
local function loadFilteredData(matchVoce, maxMatches)
    ...
    -- Cattura opportunistica del timestamp dell'ultima voce dell'ultimo file
    -- effettivamente processato in questo ciclo (sovrascritta a ogni file, quindi
    -- a fine ciclo contiene quella del file più vecchio raggiunto). Affidabile
    -- come "timestamp più vecchio della cache" SOLO se reachedEnd risulta true
    -- (vedi sotto): altrimenti lo streaming si è fermato prima per maxMatches
    -- raggiunto e potrebbe non aver mai letto l'ultimo file reale.
    local ultimoTimestampCache = nil
    ...
    while i <= 100 and not done do
        ...
        if not dataPage or not dataPage.exists then
            -- Fine naturale della cache raggiunta senza mai fermarsi per
            -- maxMatches: tutti i file esistenti sono stati processati per
            -- intero, quindi ultimoTimestampCache è genuinamente il più
            -- vecchio dell'intera cache (equivalente a getLastCacheTimestamp,
            -- ma senza un secondo require costoso sull'ultimo file).
            reachedEnd = true
            break
        end
        local success, data = pcall(function() return require(pageName) end)
        if success and type(data) == 'table' then
            ...
            if vociList and type(vociList) == 'table' and #vociList > 0 then
                ...
                filesLoaded = filesLoaded + 1
                -- Analogo a primoTimestampCache ma sull'ultima voce del file
                -- corrente: sovrascritto a ogni file, sopravvive quello dell'ultimo
                -- file raggiunto quando il ciclo termina.
                ultimoTimestampCache = effectiveTimestamp(vociList[#vociList])
                for _, voce in ipairs(vociList) do
                    if voce.titolo ~= nil then
    ...
    return { voci = filtered, ultimo_aggiornamento = ultimoAggiornamento,
        ...
        cache_esaurita = reachedEnd, ultimo_timestamp_cache = ultimoTimestampCache }
end
...
function p.main(frame)
    ...
    local dataMassima = normFilter(args.datamassima)
    ...
    local debugMode = args.debug or ''
    debugMode = mw.text.trim(mw.ustring.lower(debugMode))
    local useDebug = (debugMode == 'on' or debugMode == 'true' or debugMode == '1' or debugMode == 'yes')
    ...
    local function voceMatches(voce)
        ...
        if useAndLogic then
            ...
            if matches and dataFine ~= '' and not isAfterDate(effectiveTimestamp(voce), dataFine) then
                matches = false
            end
            if matches and dataMassima ~= '' and not isBeforeDate(effectiveTimestamp(voce), dataMassima) then
                matches = false
            end
            ...
            if dataFine ~= '' and not isAfterDate(effectiveTimestamp(voce), dataFine) then matches = false end
            if matches and dataMassima ~= '' and not isBeforeDate(effectiveTimestamp(voce), dataMassima) then matches = false end
    ...
    if not data or data.num_files == 0 then
        ...
        msg = msg .. '</div>'
        if useDebug then
            msg = msg .. '<div class="noprint" style="margin-top:0.5em; padding:0.4em 0.6em; '
                .. 'border:1px dashed #888; background:#f4f4f4; font-family:monospace; '
                .. 'font-size:85%; column-span:all;">DEBUG VociRecenti: streaming=' .. tostring(useStreaming)
                .. ' | order=' .. order .. ' | fileLettiQuestaChiamata=0 (cache assente/vuota)</div>'
        end
        return msg
    end
    ...
    -- totalCacheFiles: calcolato (solo mw.title.exists, economico) se serve per
    -- l'override di effectiveNumFiles/cacheDaysOverride (come prima) oppure se
    -- richiesto solo a scopo diagnostico da Debug=on. Nessun secondo loop:
    -- un'unica chiamata a countExistingCacheFiles() copre entrambi i casi.
    local totalCacheFiles = nil
    if useStreaming and (timestampParam == 'on' or timestampParam == 'cachedays' or useDebug) then
        ...
        if timestampParam == 'cachedays' then
            ...
            -- Se lo streaming ha già attraversato l'intera cache (num alto,
            -- nessun early-stop per maxMatches), il timestamp dell'ultima voce
            -- dell'ultimo file è già stato catturato gratuitamente durante il
            -- ciclo: lo si riusa invece di rifare un require costoso sullo
            -- stesso file appena rilasciato. Altrimenti (caso tipico, num
            -- piccolo) si ricade sul require mirato di sempre.
            local tsVecchio
            if data.cache_esaurita and data.ultimo_timestamp_cache then
                tsVecchio = data.ultimo_timestamp_cache
            else
                tsVecchio = getLastCacheTimestamp(lastIndex)
            end
    ...
    elseif not useStreaming and useDebug then
        -- Path non-streaming (loadAllData): il conteggio file esistenti serve
        -- solo per il confronto diagnostico, non altera effectiveNumFiles
        -- (che qui coincide correttamente con data.num_files).
        totalCacheFiles = (countExistingCacheFiles())
    end
    ...
    -- Riga di debug (costruita solo se useDebug: costo zero quando disattivato).
    -- Visibile nell'output renderizzato (non richiede "Visualizza sorgente").
    -- Costruita qui (dopo filtered/sort) per poter includere vociFiltrate.
    local debugLine = nil
    if useDebug then
        local dbg = {}
        table.insert(dbg, 'streaming=' .. tostring(useStreaming))
        table.insert(dbg, 'order=' .. order)
        table.insert(dbg, 'timestamp=' .. timestampParam)
        table.insert(dbg, 'fileLettiQuestaChiamata=' .. tostring(data.num_files))
        table.insert(dbg, 'fileTotaliCache=' .. tostring(totalCacheFiles or 'n/d'))
        table.insert(dbg, 'vociCaricate=' .. tostring(#(data.voci or {})))
        table.insert(dbg, 'vociFiltrate=' .. tostring(#filtered))
        table.insert(dbg, 'numRichiesto=' .. tostring(num))
        table.insert(dbg, 'cacheDaysOverride=' .. tostring(cacheDaysOverride or 'n/d'))
        debugLine = '<div class="noprint" style="margin-top:0.5em; padding:0.4em 0.6em; '
            .. 'border:1px dashed #888; background:#f4f4f4; font-family:monospace; '
            .. 'font-size:85%; column-span:all;">DEBUG VociRecenti: '
            .. table.concat(dbg, ' | ') .. '</div>'
    end
...
function p.debugVoce(frame)
    ...
    table.insert(output, "* Versione modulo: 8.53\n")
    for i, tmpl in ipairs(templates) do
        local t1 = tmpl[1]
        local t2 = tmpl[2]
