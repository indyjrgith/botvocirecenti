...
--                  ExclIfNotCat=Morti a Parigi|Parigi
--                    -> esclude chi ha "Morti a Parigi" ma non ha nessun'altra
--                       categoria contenente "Parigi" (inclusa quella stessa,
--                       quindi di fatto: esclude solo chi ha SOLO "Morti a Parigi"
--                       come legame con Parigi — attenzione: Y viene cercato
--                       sull'intero array categorie inclusa X stessa; usare
--                       una Y più specifica come "Nati a Parigi" per evitare
--                       che X soddisfi anche Y)
    ...
    for _, group in ipairs(groups) do
        ...
        for _, term in ipairs(terms) do
            ...
            if term ~= '' then
                ...
                if isNot then
                    -- not(X): soddisfatto se NESSUNA categoria matcha X
                    local anyMatch = false
                    for _, cat in ipairs(categories) do
                        if contains(cat, inner) then anyMatch = true; break end
                    end
    ...
    for _, group in ipairs(groups) do
        ...
        for _, term in ipairs(terms) do
            ...
            if term ~= '' then
                local isNot, inner = parseNot(term)
                if isNot then
                    -- not(X): la voce deve avere X; se non ce l'ha, escludila
                    local hasX = false
                    for _, cat in ipairs(categories) do
                        if contains(cat, inner) then hasX = true; break end
                    end
                    if not hasX then return false end
                else
...
local function matchesExclIfNotCat(categories, param)
    ...
    for _, rule in ipairs(rules) do
        ...
        if rule ~= '' then
            local pipe = mw.ustring.find(rule, '|', 1, true)
            if pipe then
                local triggerStr   = mw.text.trim(mw.ustring.sub(rule, 1, pipe - 1))
                local exceptionStr = mw.text.trim(mw.ustring.sub(rule, pipe + 1))
...
local function matchesPortali(templates, portali)
    ...
    for spec in (portali .. ','):gmatch('([^,]*),') do
        ...
        if portale ~= '' then
            ...
            if isNot then
                -- not(X): la voce NON deve avere questo portale
                for _, p in ipairs(allPortaleParams) do
                    if contains(p, inner) then return false end
                end
            else
                local found = false
                for _, p in ipairs(allPortaleParams) do
...
local function matchesOrPortali(templates, portali)
    ...
    for spec in (portali .. ','):gmatch('([^,]*),') do
        ...
        if portale ~= '' then
            local isNot, inner = parseNot(portale)
            if isNot then
                -- not(X): match se la voce NON ha questo portale
                local found = false
                for _, p in ipairs(allPortaleParams) do
                    if contains(p, inner) then found = true; break end
                end
    ...
    for _, voce in ipairs(data.voci) do
        ...
        if useAndLogic then
            ...
            elseif not hasPositiveFilters then
                ...
                matches = passNoCat and passNoTmpl
            else
...
function p.debugVoce(frame)
    ...
    table.insert(output, "* Versione modulo: 8.27\n")
    for i, tmpl in ipairs(templates) do
        local t1 = tmpl[1]
        local t2 = tmpl[2]
